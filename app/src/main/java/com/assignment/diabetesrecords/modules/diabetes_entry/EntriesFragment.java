...
//-------------------------------------
    ...
    private boolean validation()
    {
        ...
        {
            result=false;
            etReading.setError("Entrez une valeur");
        }
        else
        {
            ...
            {
                result=false;
                etReading.setError("Valeur incorrecte");
            }
            else
            {
        ...
        {
            result=false;
            dateet.setError("Entrez la date");
        }
        else
        {
    ...
    private boolean validerInputsTime()
    {
        boolean result=false;
        String FoodTakenStatus= spReadingTimings.getSelectedItem().toString();
        if(FoodTakenStatus.indexOf("Petit")>0)
        {
            result = MyValidator.isPetitTime(iHrs);
            if(result==false)
            Toast.makeText(getActivity(), "Heure du petit-dejeuner invalide" , Toast.LENGTH_LONG).show();

        }
        else if(FoodTakenStatus.indexOf("Dejeuner")>0)
        {
            result = MyValidator.isDejeunerTime(iHrs);

            if(result==false)
            Toast.makeText(getActivity(), "Heure du dejeuner invalide" , Toast.LENGTH_LONG).show();
        } if(FoodTakenStatus.indexOf("Diner")>0)
        {
            result = MyValidator.isDinerTime(iHrs);

            if(result==false)
            Toast.makeText(getActivity(), "Heure du diner invalide" , Toast.LENGTH_LONG).show();
        }
        return result;
    }
